...
export function BoardRefreshControl() {
  ...
  // Monitorea se qualquer query de leads está buscando dados
  const isFetching = useIsFetching({ queryKey: ['leads'] });
  const [lastUpdated, setLastUpdated] = useState<number>(Date.now());

  useEffect(() => {
    // Inscreve-se no cache para atualizar o timestamp quando qualquer query de leads tiver sucesso
    ...
    // Sincronização inicial com o estado atual do cache
