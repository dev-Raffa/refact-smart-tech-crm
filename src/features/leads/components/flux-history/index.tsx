...
export const LeadFluxHistory = ({ flowSteps }: LeadFluxHistoryProps) => {
  const stepsByFlow = flowSteps.reduce(
    (acc, item) => {
      const current = acc[item.flowName];
      if (
        !current ||
        new Date(item.executedAt) > new Date(current.executedAt)
      ) {
      ...
      <div className="relative px-2">
        ...
        {sortedSteps.length > 0 ? (
          sortedSteps.map((step, index) => {
            ...
            const statusConfig =
              STATUS_MAP[step.status as TLeadExecutionStatus];
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      statusConfig.text === 'Rodou com falha' ? (
                        <Tooltip>
                          ...
                          >
                            <p className="text-sm">
                              {step.technicalResponseDetails?.description ||
