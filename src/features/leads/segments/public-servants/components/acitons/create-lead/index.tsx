...
import { useLeadFiltersValuesOptionsQuery } from '@/features/leads/hooks/use-queries';

interface CreatePublicServantLeadSheetProps {
  trigger?: React.ReactNode;
}

export function CreatePublicServantLeadSheet({
  trigger
}: CreatePublicServantLeadSheetProps) {
  const { form, onSubmit, isSubmitting, open, setOpen } = useCreateLeadForm();

  const { data: filterOptions, isLoading: isLoadingOperators } =
    useLeadFiltersValuesOptionsQuery('PublicServant');
