...
export const importsColumns: ColumnDef<ImportedFile>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="max-w-64 text-sm truncate">{row.getValue('filePath')}</div>
    ),
  },
  {
    ...
    cell: ({ row }) => (
      <div className="flex text-sm truncate">{formatToBrNumber(row.getValue('totalRows'))}</div>
    ),
  },
  {
    ...
    cell: ({ row }) => (
      <div className="text-sm truncate">{formatToBrNumber(row.getValue('totalRowsImported'))}</div>
    ),
  },
  {
  ...
  {
    accessorKey: 'status',
    header: () => <div className="hidden sm:flex sm:justify-center">Status</div>,
    cell: ({ row }) => {
        ...
        <div className="max-w-36 mx-auto flex justify-center items-center">
          <Badge
            className={cn(
              ...
              status === 'Concluded' && 'text-emerald-600 text-[13px] font-medium dark:text-emerald-400',
              status === 'InProgress' && 'text-blue-600 text-[13px] font-medium dark:text-blue-400',
              status === 'Failed' && 'text-red-600 text-[13px] font-medium dark:text-red-400 truncate',
            )}
          >
            <span
              className={cn(
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const sourceType = row.original.fileType === 'Customers' ? 'Clientes' : 'Convênio';
